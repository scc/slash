...
sub templateEdit {
	...
	if($form->{templatedelete}) {
		...
		# don't bitch about this hack
		undef $slashdb->{_getDescriptions_templatesbypage};
		undef $slashdb->{_getDescriptions_templatepages};

		my $templates = $slashdb->getDescriptions('templatesbypage', $seclev, $page);
		my $pages = $slashdb->getDescriptions('templatepages', $seclev);
