...
[ ADD BIOLERPLATE CODE FOR MODULES, COMMAND LINE, AND WEB PROGRAMS
LATER TODAY ]
...
If you don't know how to use these modules, learn.
	...
	mkpath("/path/to/my/installation");	# right
...
user-supplied text is to be printed where HTML needs to be sripped out.
...
in the parameter portion of a URL, instead of the entire URL.
Example:
...
Slash is the name of the project.  slash is, optionally, the specific
name for the actual file distribution.  That's it.  slashcode.com or
Slashcode is the name of the site.
...
hashes or array and hash references.
...
you should submit any changes as patches on the SourceForge patches
page.
...
Use SourceForge for I<any> bug that is not being fixed immediately.  If
it is not in SourceForge, there is a good chance it will be forgotten.

Upload patches to SourceForge, too.  Use C<diff -u> for patches.
