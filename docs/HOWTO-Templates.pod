...
=head1 Can I set my own Template options?

As of now, the only way to do it is to add them to the Slash/Display.pm file,
which changes them for everyone.  We will likely be adding some variables
to the vars table to set your own preferences.


=head1 Why does whitespace keep disappearing between Template tags?

We have the POST_CHOMP, PRE_CHOMP, and TRIM options turned on.
L<Template::Context>.  L<Template::Parser>.  TRIM essentially removes leading
and trailing whitespace from the output of templates and BLOCKs.  So if you
INCLUDE a template or BLOCK, there will be no leading or trailing whitespace.
PRE_CHOMP and POST_CHOMP do the same, but for the beginning or the end of any
Template tag.  So "[% foo %] bar" is rendered as "[% foo %]bar".

TRIM cannot be disabled without changing the value in Slash/Display.pm.
PRE_CHOMP and POST_CHOMP can be disabled with the "+" sign placed immediately
after (for PRE) or before (for POST) the tag delimiter.  For example,
to keep the whitespace inthe example above: "[% foo +%] bar".


=head1 I have a really cool template for a theme/plugin, and I would like
