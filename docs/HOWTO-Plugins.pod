...
(for example, backgroun/foreground colors, calling
header and footer components, etc. will be handled
by the installed theme).

=head1 How do I get my plugin into the install system?
...
that get installed from the Makefile (this is the Makefile that
comes with the distribution).

=head1 What does my plugin need to make it work with the install?
...
see your plugin. The syntax is that each keyword must
appear on a line by itself with a '=' seperating it
and the directive. The following are the keywords it knows:
...
=head1 How do I install a plugin once my site is installed?

Use the install-plugin tool. If you call it with a -? it
will give you its basic syntax. If run it will list
what plugins it knows about and you can then select
which plugins you want (just like the install).

=head1 Do install-plugin and install-slashsite use the same logic?

Yes, you can find it in the Slash::Install library.

=head1 I have a really cool plugin and I would like to see it added to the Slash
